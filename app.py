...
from main import (
    ...
    step_2_transcribe,
    step_3_clean_text,
    step_4_build_features,
    step_5_fit_or_load_model,
    step_6_apply_scoring_with_comments,
    step_7_save_artifacts,
)
...
def run_pipeline(job_id: str, input_path: str):
    ...
    try:
        ...
        job_output_file, metrics = step_7_save_artifacts(df, job_id)
...
def upload_file():
    ...
    if file:
        ...
        return jsonify({"job_id": job_id, "message": "Файл загружен, пайплайн запущен"})
...
def calculate_metrics():
    ...
    return jsonify(
        {"job_id": job_id, "message": "Запущен расчёт метрик на исходном датасете"}
    )
